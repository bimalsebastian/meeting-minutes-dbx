...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-[var(--panel-elevated)] border border-[var(--separator)] rounded-lg text-center">
        <p className="text-[var(--text-secondary)]">No summary content available.</p>
        <p className="text-[var(--text-secondary)] text-sm mt-1">Try generating a new summary.</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        <div
          className="fixed z-50 bg-[var(--panel-bg)] shadow-lg rounded-lg py-1 min-w-[160px] border border-[var(--separator)]
      ...
      {/* <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="px-2 py-1 text-sm bg-[var(--panel-elevated)] hover:bg-gray-200 rounded-md flex items-center space-x-1"
          >
            <span>📋</span>
            <span>Copy</span>
          </button>
