        ...
        <button 
          ...
          className={`flex items-center justify-center mb-2 cursor-pointer border-none transition-colors ${
            isCollapsed 
              ...
              : "w-full px-3 py-1.5 mt-1 text-sm font-medium text-[var(--text-primary)] bg-[var(--panel-elevated)] hover:bg-gray-200 rounded-lg shadow-sm"
          }`}
          title="About Meetily"
        >
          <InfoIcon className={`text-[var(--text-secondary)] ${isCollapsed ? "w-5 h-5" : "w-4 h-4"}`} />
          {!isCollapsed && (
            <span className="ml-2 text-sm text-[var(--text-primary)]">About</span>
          )}
        </button>
      </DialogTrigger>
      <DialogContent>
