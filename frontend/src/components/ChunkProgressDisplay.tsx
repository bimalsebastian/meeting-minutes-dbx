...
export function ChunkProgressDisplay({
  ...
  const getChunkStatusColor = (status: ChunkStatus['status']) => {
    switch (status) {
      ...
      case 'pending':
      default:
        return 'text-[var(--text-secondary)] bg-[var(--panel-elevated)] border-[var(--separator)]';
    }
  };

  return (
    <div className={`bg-[var(--panel-bg)] border border-[var(--separator)] rounded-lg p-4 ${className}`}>
      {/* Progress Header */}
      ...
      <div className="mb-4">
        ...
        <div className="w-full bg-[var(--panel-elevated)] rounded-full h-2">
          <div
          ...
          {progress.chunks
            ...
            .map((chunk) => (
              ...
              >
                ...
                {chunk.text_preview && (
                  <div className="mt-1 text-[var(--text-primary)] text-xs truncate">
                    "{chunk.text_preview}"
                  </div>
                )}
...
export function ChunkProgressMini({ progress, className = '' }: { progress: ProcessingProgress; className?: string }) {
  ...
  return (
    <div className={`bg-[var(--panel-elevated)] border border-[var(--separator)] rounded-lg p-3 ${className}`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-[var(--text-primary)]">
          Processing
        </span>
      ...
      <div className="w-full bg-[var(--panel-elevated)] rounded-full h-1.5 mb-2">
        <div
