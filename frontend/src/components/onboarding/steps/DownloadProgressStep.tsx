...
export function DownloadProgressStep() {
  ...
  const renderDownloadCard = (
    ...
    <div className="bg-[var(--panel-bg)] rounded-xl border border-[var(--separator)] p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-[var(--panel-elevated)] flex items-center justify-center">
            {icon}
          </div>
          <div>
            <h3 className="font-medium text-[var(--text-primary)]">{title}</h3>
            <p className="text-sm text-[var(--text-secondary)]">{modelSize}</p>
          </div>
        </div>
        <div>
          {state.status === 'waiting' && (
            <span className="text-sm text-[var(--text-secondary)]">Waiting...</span>
          )}
        ...
        <AnimatePresence>
          {parakeetDownloaded && !summaryModelDownloaded && (
            <motion.div
              ...
              className="w-full max-w-lg bg-[var(--panel-elevated)] rounded-lg p-4 text-sm text-[var(--text-primary)]"
            >
              <div className="flex items-start gap-3">
                <Download className="w-5 h-5 text-[var(--text-secondary)] flex-shrink-0 mt-0.5" />
                <div>
