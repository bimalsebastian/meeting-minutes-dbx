  ...
  return (
    <div
      className={cn(
        ...
        isAuthorized ? 'border-gray-900 bg-[var(--panel-elevated)]' : isDenied ? 'border-red-300 bg-red-50' : 'bg-[var(--panel-bg)] border-neutral-200'
      )}
    >
      {/* Left side: Icon + Info */}
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div
          className={cn(
            ...
            isAuthorized ? 'bg-[var(--panel-elevated)]' : isDenied ? 'bg-red-100' : 'bg-neutral-50'
          )}
        >
          <div className={cn(isAuthorized ? 'text-[var(--text-primary)]' : isDenied ? 'text-red-500' : 'text-neutral-500')}>{icon}</div>
        </div>
