...
function DownloadToastContent({
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-[var(--panel-bg)] rounded-lg shadow-lg border border-[var(--separator)] p-3 relative">

      {/* Icon */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-green-100' : hasError ? 'bg-red-100' : isCancelled ? 'bg-[var(--panel-elevated)]' : 'bg-[var(--panel-elevated)]'
        }`}>
        {isComplete ? (
          ...
          <ArrowBigDownDash className="size-5 text-[var(--text-secondary)] " />
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasError ? (
          ...
          <p className="text-xs text-[var(--text-secondary)]">Download cancelled</p>
        ) : (
          <>
            {/* Progress bar */}
            <div className="w-full h-1.5 bg-[var(--panel-elevated)] rounded-full overflow-hidden mb-1.5">
              <div
