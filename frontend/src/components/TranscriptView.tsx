...
export const TranscriptView: React.FC<TranscriptViewProps> = ({ transcripts, isRecording = false, isPaused = false, isProcessing = false, isStopping = false, enableStreaming = false }) => {
  ...
  return (
    <div className="px-4 py-2">
      ...
      <AnimatePresence>
        {isRecording && (
          <div className="sticky top-4 z-10 bg-[var(--panel-bg)] pb-2">
        ...
        return (
          ...
          >
            <div className="flex items-start gap-2">
              <Tooltip>
                <TooltipTrigger>
                  <span className="text-xs text-[var(--text-secondary)] mt-1 flex-shrink-0 min-w-[50px]">
              ...
              <div className="flex-1">
                {isStreaming ? (
                  ...
                  <div className="bg-[var(--panel-elevated)] border border-[var(--separator)] rounded-lg px-3 py-2">
                    <div className="relative">
                      <p className="text-base text-[var(--text-primary)] leading-relaxed" style={{ visibility: 'hidden' }}>
                        {sizerText}
                      </p>
                      <p className="text-base text-[var(--text-primary)] leading-relaxed absolute top-0 left-0">
                        {displayText}
                      </p>
                    </div>
                  </div>
      ...
      {!isStopping && isRecording && !isPaused && !isProcessing && transcripts.length > 0 && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="flex items-center gap-2 mt-4 text-[var(--text-secondary)]"
        >
