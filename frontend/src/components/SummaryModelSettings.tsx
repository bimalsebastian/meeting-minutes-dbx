  ...
  const fetchModelConfig = useCallback(async () => {
    try {
      ...
      if (data && data.provider !== null) {
        ...
        if (data.provider !== 'ollama' && data.provider !== 'builtin-ai' && data.provider !== 'databricks' && !data.apiKey) {
          try {
    ...
    <div className='flex flex-col gap-4'>
      <div className="bg-[var(--panel-bg)] rounded-lg border border-[var(--separator)] p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-2">Auto Summary</h3>
            <p className="text-sm text-[var(--text-secondary)]">Auto Generating summary after meeting completion(Stopping)</p>
          </div>
